...
double SVD_Sim_M::computeSim(uInt id1, uInt id2)
{
    ...
    double len1 = 0;
    double len2 = 0;
    for(uInt i = 0; i < pSVD->feature_num; i ++)
    {
        svd_sim += pSVD->movieFeatures[id1 - 1][i] * pSVD->movieFeatures[id2 - 1][i];
        len1 += pow(pSVD->movieFeatures[id1 - 1][i], 2);
        len2 += pow(pSVD->movieFeatures[id2 - 1][i], 2);
    }
